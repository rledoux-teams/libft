...
/*   Updated: 2023/01/17 18:18:12 by rledoux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strdup_bonus(t_gnl *s_gnl, char *buf)
{
	...
	i = 0;
	s_gnl->save_buf = ft_calloc_bonus(s_gnl->read_size + s_gnl->start);
	...
	free(buf);
	buf = ft_calloc_bonus(s_gnl->read_size + BUFFER_SIZE + s_gnl->start + 1);
	i = 0;
	while (i < s_gnl->read_size + s_gnl->start)
	{
...
char	*ft_realloc_bonus(t_gnl *s_gnl, char *buf)
{
	...
	{
		buf = ft_calloc_bonus(BUFFER_SIZE);
		return (buf);
	}
	buf = ft_strdup_bonus(s_gnl, buf);
	if (s_gnl->read_size > 0)
		buf = ft_strjoin_bonus(s_gnl, buf);
	return (buf);
}
